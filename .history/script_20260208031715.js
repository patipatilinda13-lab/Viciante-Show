...
function entrarNaSala(idSala) {
  ...
  // simulando jogador atual
  const jogadorAtual = "Lucas";

  if (!sala.jogadores.includes(jogadorAtual)) {
    if (sala.jogadores.length < sala.limite) {
      sala.jogadores.push(jogadorAtual);
    }
  }
  ...
  salaAtual = sala;
  adminLogado = false; // Reset admin ao entrar na sala
...
function renderizarParticipantesComCheckbox() {
  ...
  salaAtual.jogadores.forEach(jogador => {
    ...
    const pagouTexto = salaAtual.pagamentos[jogador] ? "✅ Pagou" : "⏳ Aguardando";
    valor.textContent = `${pagouTexto} - Ingresso: R$${salaAtual.valor}`;
