...
let contadorJogadoresEntrada = 0; // BLOQUEADO: Contador para nomes "Lucas 1", "Lucas 2", etc
...
function sincronizarAtualizacoes() {
  // Sincronizar quando outra aba muda o localStorage
  window.addEventListener("storage", (e) => {
    if (e.key === CHAVE_SALAS_STORAGE) {
      carregarSalas();
      // Apenas renderizar se estamos na tela de salas
  ...
  // Polling para caso de mesma aba (não dispara storage event)
  setInterval(() => {
    carregarSalas();
    // Apenas renderizar se estamos na tela de salas
    if (telaSalas.style.display !== "none") {
...
function renderizarSalas() {
  ...
  salas.forEach(sala => {
    ...
    btn.onclick = () => entrarNaSala(sala.id);
    div.appendChild(btn);
...
function entrarNaSala(idSala) {
  ...
  // BLOQUEADO: Autenticação desabilitada
  // POR ENQUANTO: Gerar nomes automaticamente "Lucas 1", "Lucas 2", etc
  contadorJogadoresEntrada++;
  nomeJogadorAtual = `Lucas ${contadorJogadoresEntrada}`;
  
  // MODO BLOQUEADO: Gerar ID NOVO a cada entrada (não reutilizar do localStorage)
  // Assim cada "aba" tem um ID diferente
  idJogadorAtual = "user_" + Math.random().toString(36).substr(2, 9) + "_" + Date.now();
...
// ========== INICIALIZAÇÃO ==========
// BLOQUEADO POR ENQUANTO: Sistema de autenticação desabilitado para testes
// Futuramente será reativo
function inicializar() {
  carregarSalas();
  
  // ========== DESABILITADO: Verificar se já tem usuário logado ==========
  // const usuarioSalvo = localStorage.getItem(CHAVE_USUARIO_LOGADO);
  // if (usuarioSalvo) { ... }
  
  // POR ENQUANTO: Ir direto para salas sem autenticação
  usuarioLogadoAtual = "Visitante";
  ...
  // Forçar recarga das salas antes de renderizar
  carregarSalas();
  renderizarSalas();
  
  verificarSessaoPersistida();
  
  // Iniciar sincronização DEPOIS da tela estar pronta
